...
histogram beta, title("Coefficient Beta across households - Urban", color(black)) xtitle ("Betas") graphregion(color(white)) bcolor(blue)
graph export "C:\Users\Usuario\Desktop\CEMFI\Term 5\Development\HA\HA 3\HistBetaUrb.png", replace 
restore
...
histogram phi, title("Coefficient Phi across households - Urban", color(black)) xtitle ("Phis") graphregion(color(white)) bcolor(pink) 
graph export "C:\Users\Usuario\Desktop\CEMFI\Term 5\Development\HA\HA 3\HistPhiUrb.png", replace 
restore
 ...
 * Define five income groups
 sort income_ip
 gen nobs = _N // total number of observations
 gen nhh = _n 
 
 gen income_group = 0
 replace income_group = 1 if nhh<=122
 replace income_group = 2 if nhh>122 & nhh<=244
 replace income_group = 3 if nhh>244 & nhh<=365
 replace income_group = 4 if nhh>365 & nhh<=487
 replace income_group = 5 if nhh>487 & nhh<=609
